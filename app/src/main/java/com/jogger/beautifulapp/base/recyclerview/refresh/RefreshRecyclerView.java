...
public class RefreshRecyclerView extends RecyclerView {
    ...
    private void restoreRefreshView() {
        ...
        final int currentTopMargin = ((MarginLayoutParams) mRefreshView.getLayoutParams())
                .topMargin;
        if (currentTopMargin > mRefreshViewHeight / 2) {
            //先弹到height/2，然后做刷新操作
            ValueAnimator animator = ObjectAnimator.ofFloat(currentTopMargin, mRefreshViewHeight
                    / 2)
                    .setDuration(500);
            animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                ...
                public void onAnimationUpdate(ValueAnimator animation) {
                    ...
                    updateRefreshStatus((int) currentTopMargin);
                }
            });
            animator.start();
            ...
            endRefresh(currentTopMargin);
        }
    }

    private void endRefresh(int currentTopMargin) {
        if (mRefreshCreator != null) {
            mRefreshCreator.onPull(currentTopMargin, mRefreshViewHeight, mCurrentRefreshStatus);
        }
    ...
    private void updateRefreshStatus(int marginTop) {
        ...
        } else if (marginTop < SizeUtil.getScreenHeight()) {//设置最大滑动距离
    ...
    private void addRefreshView() {
        ...
        if (adapter != null && mRefreshCreator != null) {
            ...
            if (refreshView != null) {
                ((BaseQuickAdapter) adapter).addHeaderView(refreshView,0);
    ...
    public void onStopRefresh() {
        if (mCurrentRefreshStatus != REFRESH_STATUS_REFRESHING) return;
        mCurrentRefreshStatus = REFRESH_STATUS_NORMAL;
        restoreRefreshView();
        if (mRefreshCreator != null) {
